...
import type { Component, QObject as NodeObject, QWidget as NodeWidget, QObjectSignals, QWidgetSignals } from '@nodegui/nodegui';
import type { RNComponent } from "../../react-nodegui/src/components/config";
import { warn, error, log } from '../../shared/Logger';
import type { EventWidget } from '@nodegui/nodegui/dist/lib/core/EventWidget';
import { type QVariantType } from '@nodegui/nodegui/dist/lib/QtCore/QVariant';
...
export abstract class NSVNode {
    ...
    checkValidTrace(message = '') {
        const childs = this.childNodes
        const length = childs.length
        if (length <= 1) return
        console.log('VALID NODE CHECK :: ' + message)
        for (let i = 0; i < length; i++) {
            const pre = childs[i - 1]
            const next = childs[i + 1]
            const child = childs[i]
            if (pre != child.previousSibling) return this.showMatrixNode(), console.log('pre err: ' + i)
            if (next != child.nextSibling) return this.showMatrixNode(), console.log('next err' + i)
        }
    }
    showMatrixNode() {
        const childs = this.childNodes
        const length = childs.length
        let matrix = [`----Maxtrix::${length}----`];
        for (let i = 0; i < length; i++) {
            const pre = childs[i - 1]
            const next = childs[i + 1]
            const child = childs[i]
            matrix.push(`[${i}][${child.nodeId}][${pre?.nodeId || -1}::${child.previousSibling?.nodeId || -1}]` +
                `[${next?.nodeId || -1} => ${child.nextSibling?.nodeId || 'INF'}][${child}]`)
        }
        matrix.push(`------------`)
    ...
    appendChild(child: NSVNode): NSVNode {
        ...
        const pre = this.childNodes[this.childNodes.length - 1] ?? null

        if (pre) {
            child.previousSibling = pre;
            pre.nextSibling = child
            // console.log(`append child ${child} :: ${pre}`)
        }
    ...
    insertBefore(newNode: NSVNode, referenceNode: NSVNode): NSVNode {
        ...
        let referenceIndex = referenceNode == null ?
            -1 :
        ...
        if (referenceIndex === -1) {
            ...
            // If the child is already a child of this, remove it from our childNodes array before adding it to the end.
            let newNodeIndex = -1
            while (newNode.parentNode === this) {
                newNodeIndex = this.childNodes.findIndex(c => c === newNode);
                if (newNodeIndex === -1) {
                    throw new Error("NotFoundError: the object can not be found here.");
                }
                childs.splice(newNodeIndex, 1);
                const lastIndex = childs.length - 1
                if (lastIndex === -1) break
                if (lastIndex === 0 || newNodeIndex === 0) {
                    childs[0].previousSibling = null
                    break
                }
                if (newNodeIndex > lastIndex) {
                    childs[lastIndex].nextSibling = null
                    break
                }
                childs[newNodeIndex - 1].nextSibling = childs[newNodeIndex]
                childs[newNodeIndex].previousSibling = childs[newNodeIndex - 1]
                break
            }

            if (newNodeIndex !== -1 && newNodeIndex < referenceIndex) --referenceIndex
            this.childNodes.splice(referenceIndex, 0, newNode);

            const refPre = referenceNode.previousSibling
            newNode.nextSibling = referenceNode
            referenceNode.previousSibling = newNode
            if (refPre) {
                newNode.previousSibling = refPre
                refPre.nextSibling = newNode
            }
            ...
            // console.log(`${this.childNodes[referenceIndex] === referenceNode}${referenceNode}`)
    ...
    removeChild(child: NSVNode): NSVNode {
        // this.checkValidTrace('remove child 1')
        if (child === this) {
            throw new Error("HierarchyRequestError: The operation would yield an incorrect node tree.");
        }
        if (child instanceof NSVNode === false) {
        ...
        if (childIndex !== -1) {
            ...
            while (true) {
                ...
                childs[childIndex - 1].nextSibling = childs[childIndex]
                childs[childIndex].previousSibling = childs[childIndex - 1]
                break
            }
        ...
        // this.checkValidTrace('remove child 2')
        return child;
    }
}
...
export class NSVElement<T extends NativeView = NativeView> extends NSVNode {
    ...
    public readonly style = {
        ...
        get cssText(): string {
            ...
            return this.n().nativeView.styleSheet()
        },
        ...
        set cssText(value: string) {
            ...
            this.n().nativeView.setStyleSheet(value)
        },
        n: () => this as any
    }
    ...
    insertBefore(newNode: NSVNode, referenceNode?: NSVNode | null): NSVNode {
        ...
        // this.checkValidTrace('insert before 1')
                ...
                if (newElement.nativeView) {
                    if (referenceNode) {
                        if ((referenceNode as NSVElement).nativeView) {
                            ...
                            // console.log('find next element', node.nodeId, referenceNode.nodeId, `${node.parentNode.childNodes.length}`)
                            // fix for svelte
                            function findClosestNonVirtualNextElementSibling(node: NSVNode): NSVElement | null {
                            ...
                            const closest: NSVElement | null = findClosestNonVirtualNextElementSibling(referenceNode);
        ...
        // this.checkValidTrace('insert before 2')
        return node;
    }
    toString(): string {
